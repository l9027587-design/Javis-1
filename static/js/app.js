    ...
    try {
      ...
      comboListEl.innerHTML = combos.map((c) => `
        <div class="combo-card">
          <div class="combo-legs">${c.legs.map((leg) => `<span class="combo-leg">${leg.match ? `${leg.match} <span class="combo-leg-date">(${fmtTime(leg.start_time)})</span>: ` : ""}${leg.pick} <b>${leg.odds.toFixed(2)}</b></span>`).join(" + ")}</div>
    ...
    try {
      ...
      comboHistoryListEl.innerHTML = history.map((c) => `
        <div class="combo-card combo-history-card status-${c.status}">
          ...
          <div class="combo-legs">${c.legs.map((leg) => `<span class="combo-leg leg-${leg.status}">${leg.status === "won" ? "✓" : leg.status === "lost" ? "✗" : "…"} ${leg.match ? `${leg.match} <span class="combo-leg-date">(${fmtTime(leg.start_time)})</span>: ` : ""}${leg.pick} <b>${leg.odds.toFixed(2)}</b></span>`).join(" + ")}</div>
          <div class="combo-summary">
            <span>Quote <b>${c.combined_odds.toFixed(2)}</b></span>
            <span>Trefferchance ${(c.combined_prob * 100).toFixed(0)}%</span>
            <span class="${c.combined_ev >= 0 ? "ev-positive" : "ev-negative"}">EV ${c.combined_ev >= 0 ? "+" : ""}${(c.combined_ev * 100).toFixed(1)}%</span>
